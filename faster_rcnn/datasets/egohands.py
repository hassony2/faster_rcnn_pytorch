...
class EgoHandDataset(HandDetectionDataset):
    def __init__(self, split, transform,
        ...
        """
        super(EgoHandDataset, self).__init__('egohands_ + split',
    ...
    def _annotation_from_index(self, index):
        ...
        bboxes, overlaps, gt_classes = self.get_frame_annots(video_idx,
                                                             frame_idx)
    ...
    def get_frame_annots(self, video_idx, frame_idx):
        ...
        if len(bboxes) == 0:
            ...
            if self.left_from_right:
                ...
                left_labels = [left_hand_cls_idx for bbox in [bbox_myleft,
                                                              bbox_yourleft]
                               if bbox is not None]
                right_labels = [right_hand_cls_idx for bbox in [bbox_myright,
                                                                bbox_yourright]
                                if bbox is not None]
