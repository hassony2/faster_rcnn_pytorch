...
class LISADataset(HandDetectionDataset):
    def __init__(self,
        ...
        Args:
            ...
            transform: transform to apply to image after joint transfromations
                have been applied
            transform_params(dict): dictionnary containing the params for data
                augmentation with keys {'scale', 'hue', 'jitter',
                'exposure', 'saturation'}
        """
        ...
        self.transform_parms = transform_params

        # Set usefull paths for given split
        self.name = 'lisa' + split
        self._data_path = os.path.join(self.datadir, 'LISA_HD')
    ...
    def _annotation_from_index(self, index):
        ...
        gt_overlaps = np.zeros((num_objs, self.num_classes))
